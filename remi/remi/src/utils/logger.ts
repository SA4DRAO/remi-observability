...
const log = (level: LogLevel, message: string, data?: unknown): void => {
  ...
  if (level === "error" && config.sentry?.dsn) {
    const e = data instanceof Error ? data : undefined;
    if (e) Sentry.captureException(e, { tags: { level } });
    else Sentry.captureMessage(message, { level: "error" });
  }
};
