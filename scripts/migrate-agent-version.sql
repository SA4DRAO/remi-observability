-- =============================================================================
-- Remi Migration: add agent_version to sessions_v2 and session_rollups_v2
-- Safe to re-run (uses IF NOT EXISTS / CREATE OR REPLACE).
-- Apply with:
--   PGPASSWORD=remi_password psql -h localhost -p 5432 -U remi_user -d remi_db \
--     -f scripts/migrate-agent-version.sql
-- =============================================================================
...
CREATE INDEX IF NOT EXISTS idx_sessions_v2_agent_version
  ON sessions_v2(agent_id, agent_version);

CREATE INDEX IF NOT EXISTS idx_rollups_v2_agent_version
  ON session_rollups_v2(agent_id, agent_version);

-- Recreate stored procedure to include agent_version
...
BEGIN
  ...
  INSERT INTO session_rollups_v2 (
    session_id, org_id, agent_id, agent_version,
  ...
  SELECT
    p_session_id, sv.org_id, sv.agent_id, sv.agent_version,
